# 贡献指南

感谢参与 `xuanyituwen`。这个项目的重点不是堆叠更长的提示词，而是让悬疑图文的因果、证据和视觉连续性可以被复盘。

## 提交前检查

```bash
python -m json.tool evals/evals.json > /dev/null
python -m py_compile scripts/normalize_raster.py
```

提交前请确认：

- 没有个人聊天记录、账号信息、API 密钥或未获授权素材。
- 修改主流程时，同步更新对应的 references 文件和 README。
- 新增规则说明它解决的具体失败模式。
- 示例图、字体、声音和文本都有明确的使用权。
- 图片保持纯净无字；发布页的配文、标题、话题和置顶评论由创作者自行填写，不写回底图。

## 适合提交的问题

- 角色、场景或关键物件在连续图中发生了可复现的漂移。
- 伏笔在前文没有公平埋设，或结尾没有回收。
- 信息权限越界，角色使用了自己不可能知道的内容。
- 逐图卡、prompt 和图片事实不一致。
- 归一化脚本在特殊输入比例、透明通道或边界尺寸下失败。

## 提交方式

请先创建 issue 描述问题和复现条件，再提交小而集中的 pull request。不要把生成图片原件、个人素材或整个评测工作区提交进仓库。
